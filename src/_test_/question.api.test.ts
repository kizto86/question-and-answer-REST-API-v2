...
describe("/questions", () => {
  ...
  it("should return a single existing question", async()=>{
    const testQuestion:{username:string,question_title:string,question_description:string} = {
      username:"Mina",
  ...
  it("should support the creation of questions", () => {
    return request(app)
        ...
        .then((response:any) =>
            expect(response.body).toEqual({ message: "question created successfully" })
        );
  });
